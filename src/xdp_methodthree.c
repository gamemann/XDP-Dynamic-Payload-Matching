...
struct bpf_map_def SEC("maps") payload_map = 
{
    ...
    .max_entries = MAX_PAYLOAD_LENGTH
};
...
{    
    ...
    {
        uint8_t *byte = data + sizeof(struct ethhdr) + (iph->ihl * 4) + l4len;

        if (!(*(byte + i + 1) > (uint8_t *)data_end))
        {
            uint8_t *map = bpf_map_lookup_elem(&payload_map, byte);
